...
The [`@keystone-next/testing`](../guides/testing) package provides tools to help you run tests against your GraphQL API.

Options:

- `isolateRuns` (default: `false`): If you need to run tests against more than one config object in your tests you can set this option to `true`, which will ensure that each config is isolated in its own artifact directory and database schema.
- `artifactPath` (default: `'./keystone/tests'`): The location to write generated artifacts such as the GraphQL and Prisma schemas.
  If `isolateRuns` is `true` then artifacts will be written to a subirectory based on a hash of the schema.
