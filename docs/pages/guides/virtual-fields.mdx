...
As well as passing in the `field` definition, we have also passed in `graphQLArgs: { length: 500 }`.
...
Had we not specified `defaultValue` in our field, the `graphQLArgs` argument would be **required**, as the Admin UI would not be able to query this field without it.
...
We can set up a GraphQL type called `PostConts` to represent this data using the `schema.object()` function.
