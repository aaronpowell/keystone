...
We'll start with a list called `Example` and create a virtual field called `randomNumber`.
...
We can now run a GraphQL query and request the `randomNumber` field on one our items.
...
```javascript
{ Example: { id: "1", randomNumber: 7 } }
```

The value of `randomNumber` is generated from the `resolve` function, which returns a random number from 1 to 10.
Each time you run this query you can expect to get a different random number back.
